...
reworks compseg as mutual information (the change is in the get_bidir_transprobs function, and also eliminates Fisher's Exact test)

$ python3 loginsep.py --help
...
def get_bidir_transprobs(featpath, datapath, **kwargs):
    ...
    '''
    tempdic = nc.bidir_prob_wrapper(featpath, datapath, **kwargs)
    outdic = {}.fromkeys(tempdic[0])
    for val in outdic:
        outdic[val] = np.log2(tempdic[0][val])
    return (outdic, tempdic[1])
...
if __name__=='__main__':
    ...
    if args.language:
        ...
        try:
            ...
            msg.env_render(message=f"attempting to plot: {os.path.join(args.outdir, 'simulation')} but something went wrong. Are the simulation files at that location?")
            raise
